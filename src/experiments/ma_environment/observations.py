...
class DefaultObservationFunction(ObservationFunction):
    ...
    def observation_space(self) -> spaces.Box:
        ...
        return spaces.Box(
            low=np.zeros(self.ts.num_green_phases + 1 + 2 * len(self.ts.lanes), dtype=np.float32), # +1
            high=np.ones(self.ts.num_green_phases + 1 + 2 * len(self.ts.lanes), dtype=np.float32), # +1
        )
